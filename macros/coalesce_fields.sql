...
{# link for the Github repo: https://github.com/dbt-labs/stitch-utils/blob/main/macros/coalesce_fields.sql  #}
    ...
    {%- for group in cols_to_coalesce|groupby('name_without_datatype') %}
        ...
        {%- set column_exp -%}
            ...
            {%- for column in group.list -%}
                ...
                case
                    when {{column.name}} = true then cast('true' as {{dbt.type_string()}})
                    when {{column.name}} = false then cast('false' as {{dbt.type_string()}})
                    else null end
                {% else %}
                cast({{column.name}} as {{dbt.type_string()}}){%- endif -%}{{- "," if not loop.last -}}
            {% endfor -%}
            ) as {{ group.grouper }}
        {%- endset -%}
